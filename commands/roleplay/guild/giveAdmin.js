exports.run = async (update, args) => {
  ...
  try {
    ...
    try {
      ...
      throw new CommandError('Упомяни человека, которого хочешь назначить управляющим', 'Argument_InvalidMention')
    }
    
    // Self check
    if (memberId === senderId) {
      throw new CommandError('Нельзя давать себе роль; ты и так создатель!', 'Argument_MentionIsSelf')
    }
    ...
    return update.reply(`✨ [id${memberId}|${memberName}] был назначен управляющим по воле [id${senderId}|${userName}]!`)
