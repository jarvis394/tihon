exports.run = async (update, args) => {
  ...
  try {
    ...
    if (!data) {
      throw new CommandError(`Ты состоишь в несуществующем колхозе "${guildId}"!`, 'Guild_NotFound')
    }
    ...
    if (userRole < 2) {
      throw new CommandError('Только управляющие колхозом могут приглашать людей', 'User_MissingRole')
    }
    
    if (member) {
      if (member.role < 1) {
        throw new CommandError('Этот человек уже приглашён в ваш колхоз!', 'Guild_MemberAlreadyInvited')
      } else {
        throw new CommandError('Этот человек уже состоит в вашем колхозе!', 'Guild_MemberAlreadyIn')
      }
    }
