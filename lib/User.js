const { firebase, users: store, log, api } = require('../variables')
...
class User {
  ...
  pushGuildId(id) {
    ...
    return ref.update({ guild: id }).catch(e => log.error(e))   
  }
  /** 
  ...
  async fetchGuild() {
    ...
    else if (this.data.guild && data.guild !== this.data.guild) {
      return this.data.guild
    }
    else {
      return data.guild
   ...
   */
  async getAmount() {
    const a = (await this.fetchBalance()) + this.data.amount
    return a < 0 ? 0 : a
  }
   ...
   */
  async getReputation() {
    const a = (await this.fetchReputation()) + this.data.rank
    return a < 0 ? 0 : a
  }
